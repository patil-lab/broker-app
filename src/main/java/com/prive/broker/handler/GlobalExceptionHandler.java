...
import com.prive.broker.exception.ApplicationeError;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<BrokerServiceResult<ApplicationeError>> handleApplicationException(BrokerServiceException ex, HttpServletRequest request) {
        ApplicationeError error = ex.getError();
        LOGGER.error(ex.getMessage(),ex);

        return ResponseEntity.ok(BrokerServiceResult.fail(error));
    }

    @ExceptionHandler(HttpClientErrorException.class)
    public ResponseEntity<BrokerServiceResult> handle(HttpClientErrorException ex) {
        LOGGER.error(ex.getMessage(),ex);
        return ResponseEntity.ok(BrokerServiceResult.fail(ex.getMessage()));
    }
    ...
    private ResponseEntity<Object> createResponseEntity(String message, int code, HttpStatus status) {
        BrokerServiceException clientErrorException = new BrokerServiceException(this.serviceName, message, code);
